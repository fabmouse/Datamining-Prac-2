...
vote.rf = randomForest(Party~ Vote.1+Vote.2+Vote.3+Vote.4+Vote.5+Vote.6++Vote.7+Vote.8,data = factorVote)
...
## when mtry = 5, the error is minimum

## try to find a good tree number
set.seed(100)
fit.rf <- randomForest(factor(Party)~ Vote.1+Vote.2+Vote.3+Vote.4+Vote.5+Vote.6++Vote.7+Vote.8,data = train,mtry = 5,ntree = 1000)
plot(fit.rf)
## when ntree = 500, the model is steable

## when ntree = 500
set.seed(100)
select.rf <- randomForest(factor(Party)~ Vote.1+Vote.2+Vote.3+Vote.4+Vote.5+Vote.6++Vote.7+Vote.8,data = train,mtry = 5,ntree = 500)
print(select.rf)
## when ntree = 500, the OOB err is 3.56%
